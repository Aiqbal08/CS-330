...
const GLchar* surfaceVertexShaderSource = GLSL(400,
...
const GLchar* surfaceFragmentShaderSource = GLSL(400,
...
int main(int argc, char* argv[])
{
	...
	texFilename = "resources/textures/lime.jpg";
	if (!UCreateTexture(texFilename, SphereID))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}

	texFilename = "resources/textures/battery.png";
	if (!UCreateTexture(texFilename, PowerID))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}

	texFilename = "resources/textures/HeadPhones.png";
	if (!UCreateTexture(texFilename, HeadPhones))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}

	texFilename = "resources/textures/screen.png";
	if (!UCreateTexture(texFilename, screen))
	{
		cout << "Failed to load texture " << texFilename << endl;
		return EXIT_FAILURE;
	}
...
void URender()
{
	...
	*/
	...
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, HeadPhones);


	// 1. Scales the object
	scale = glm::scale(glm::vec3(0.60f, 0.2f, .60f));
	// 2. Rotate the object
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(0.0f, 0.0f, 3.0f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	glUniform4f(objColLoc, 0.6f, 0.6f, 0.60f, 1.0f);
	...
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(0.0f, 0.0f, 5.5f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
	...
	// 1. Scales the object
	scale = glm::scale(glm::vec3(1.6f));
	// 2. Rotate the object
	rotation = glm::rotate(glm::radians(90.0f), glm::vec3(1.0, 0.0f, 0.0f)) * glm::rotate(glm::radians(-90.0f), glm::vec3(0.0, 0.0f, 1.0f));
	// 3. Position the object
	translation = glm::translate(glm::vec3(0, 0, 4.15f));
	// Model matrix: transformations are applied right-to-left order
	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	glUniform4f(objColLoc, 0.6f, 0.6f, 0.6f, 1.0f);

	// Draws the triangles
	glDrawArrays(GL_TRIANGLES, 0, meshes.gTorusMesh.nVertices / 2.0f);
...
// Green Ball (Sphere)
	...
	scale = glm::scale(glm::vec3(0.767f)); // Size of the ball
	rotation = glm::rotate(0.0f, glm::vec3(1.0, 1.0f, 1.0f)); // No rotation
	translation = glm::translate(glm::vec3(2.0f, 1.0f, 0.0f)); // Position of the ball

	model = translation * rotation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	glUniform4f(objColLoc, 0.0f, 1.0f, 0.0f, 1.0f); // Green color
	...
	// ... [Other rendering code]
	// Rectangle Rechargeable Battery
	glBindVertexArray(meshes.gBoxMesh.vao);

	// Scale, rotate, and translate for the battery
	scale = glm::scale(glm::vec3(1.6f, 0.2f, 1.0f)); // Adjust to make it look like a battery
	translation = glm::translate(glm::vec3(-2.0f, 0.0f, -3.0f)); // Position of the battery

	model = translation * scale;
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));

	glUniform4f(objColLoc, 0.15f, 0.15f, 0.15f, 1.0f); // Color of the battery

	// bind textures on corresponding texture units
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_2D, PowerID);

	glDrawElements(GL_TRIANGLES, meshes.gBoxMesh.nIndices, GL_UNSIGNED_INT, nullptr);

	glUniform1i(uHasTextureLoc, 0);

	scale = glm::scale(glm::vec3(1.65f, 0.194f, 1.05f)); // Adjust to make it look like a battery
	translation = glm::translate(glm::vec3(-2.0f, 0.0f, -3.0f)); // Position of the battery
	...
	//phone body
	scale = glm::scale(glm::vec3(1.85f, 0.195f, 1.25f)); // Adjust to make it look like a battery
	translation = glm::translate(glm::vec3(-3.0f, 0.0f, 2.0f)); // Position of the battery
	rotation = glm::rotate(glm::radians(75.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	...
	scale = glm::scale(glm::vec3(1.8f, 0.2f, 1.2f)); // Adjust to make it look like a battery
	translation = glm::translate(glm::vec3(-3.0f, 0.0f, 2.0f)); // Position of the battery
	rotation = glm::rotate(glm::radians(75.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	...
	scale = glm::scale(glm::vec3(1.0f, 0.2f, 1.8f)); // Adjust to make it look like a battery
	translation = glm::translate(glm::vec3(0.0f, 0.0f, 4.0f)); // Position of the battery
	...
	glUniform1i(multipleTexturesLoc, 0);
...
*/
	...
	glUniform1f(specInt1Loc, 1.01f);
	glUniform1f(specInt2Loc, 1.01f);
	glUniform1f(specInt3Loc, 1.01f);
	glUniform1f(specInt4Loc, 1.01f);
	glUniform1f(specInt5Loc, 1.01f);
	//set specular highlight size
	glUniform1f(highlghtSz1Loc, 62.0f);
	glUniform1f(highlghtSz2Loc, 62.0f);
	glUniform1f(highlghtSz3Loc, 62.0f);
	glUniform1f(highlghtSz4Loc, 62.0f);
	glUniform1f(highlghtSz5Loc, 62.0f);
